...
const advancedConfigDefaults = Object.freeze({
  ...
  proxy_auto_detect: true, fixed_proxy_url: '',
...
function renderAdvancedConfig() {
  ...
  const egressProxy = [
    acToggle('proxy-auto-detect', '自动探测本地代理端口', d.proxy_auto_detect, '每次启动/保存配置时自动探测本机 Clash/FlClash/Mihomo 的 mixed-port 并覆盖 proxy-url；关闭后进入固定模式。'),
    acText('fixed-proxy-url', '固定 proxy-url', d.fixed_proxy_url, 'http://127.0.0.1:7897 或 direct', '关闭自动探测后生效：填写值将固定写入 proxy-url；留空则保留 base-config.yaml 原值（清空 base-config 里的 proxy-url 即直连）。'),
  ].join('');
  const cpaLogging = [
  ...
  content.innerHTML = [acCard('路由与会话','Routing',routing),acCard('重试与冷却','Reliability',retry),acCard('配额与服务','Service',service),acCard('响应与媒体','Response',response),acCard('协议兼容','Compatibility',compatibility),acCard('代理出口','Egress proxy',egressProxy),acCard('CPA 应用日志','CPA runtime',cpaLogging,true),acCard('Dashboard 请求监控','Dashboard monitor',requestMonitoring,true)].join('');
