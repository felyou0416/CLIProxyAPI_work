...
class RuntimeConfigRequestLogTests(unittest.TestCase):
    ...
    def test_build_runtime_config_disables_request_log_when_monitoring_is_off(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = pathlib.Path(tmp)
            base_config = root / 'base-config.yaml'
            runtime_config = root / 'runtime.yaml'
            ...
            with patch.object(auth, 'BASE_CONFIG', base_config), \
                 ...
                 patch.object(auth, 'rewrite_openai_compatibility', side_effect=lambda text, entries: text), \
                 patch.object(auth, '_runtime_config_validation_issues', return_value=[]):
                auth.build_runtime_config(
                    bind_host='127.0.0.1',
                    access_api_keys=['cliproxyapi'],
                    state={'request_monitoring_enabled': False},
                )

            runtime_text = runtime_config.read_text(encoding='utf-8')
            self.assertIn('request-log: false', runtime_text)
            self.assertNotIn('request-log: true', runtime_text)

    def test_dashboard_monitoring_options_are_not_written_to_runtime_yaml(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = pathlib.Path(tmp)
            base_config = root / 'base-config.yaml'
            runtime_config = root / 'runtime.yaml'
            ...
            runtime_text = runtime_config.read_text(encoding='utf-8')
            for key in (
                'request-observability-refresh-seconds',
                'request-events-cache-ttl-seconds',
                'request-log-keep-files',
                'request-event-archive-keep-entries',
            ):
                self.assertNotIn(key, runtime_text)
