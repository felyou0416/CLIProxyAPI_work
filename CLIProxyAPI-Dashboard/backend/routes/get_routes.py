...
def handle_get(handler, parsed):
    ...
    if parsed.path == '/api/advanced-config':
        ...
        send_json(handler, {'ok': True, 'item': {
            ...
            'request_retry': state.get('request_retry', 3),
            'max_retry_credentials': state.get('max_retry_credentials', 0),
            'max_retry_interval': state.get('max_retry_interval', 30),
            'save_cooldown_status': state.get('save_cooldown_status', False),
            'transient_error_cooldown_seconds': state.get('transient_error_cooldown_seconds', 0),
            'quota_switch_project': state.get('quota_switch_project', True),
            'quota_switch_preview_model': state.get('quota_switch_preview_model', True),
            'quota_antigravity_credits': state.get('quota_antigravity_credits', True),
            'disable_image_generation': state.get('disable_image_generation', 'off'),
            'video_result_auth_cache_ttl': state.get('video_result_auth_cache_ttl', '3h'),
            ...
            'logging_to_file': state.get('logging_to_file', False),
            'logs_max_total_size_mb': state.get('logs_max_total_size_mb', 0),
