...
func TestFileSynthesizer_Synthesize_NestedCodexAccessOnly(t *testing.T) {
	tempDir := t.TempDir()
	codexDir := filepath.Join(tempDir, "codex")
	if errMkdir := os.Mkdir(codexDir, 0755); errMkdir != nil {
	...
	filePath := filepath.Join(codexDir, "access-only.json")
	...
	auth := auths[0]
	if auth.ID != filepath.Join("codex", "access-only.json") {
		t.Fatalf("expected nested relative ID, got %s", auth.ID)
	}
	if auth.Provider != "codex" {
		t.Fatalf("expected provider codex, got %s", auth.Provider)
	}
	if got, _ := auth.Metadata["access_token"].(string); got != "access-token" {
		t.Fatalf("expected access token to be promoted, got %q", got)
	}
	if got, _ := auth.Metadata["refresh_token"].(string); got != "" {
