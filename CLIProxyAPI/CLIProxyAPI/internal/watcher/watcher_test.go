...
func TestLoadFileClientsCountsNestedAuthFiles(t *testing.T) {
	authDir := t.TempDir()
	codexDir := filepath.Join(authDir, "codex")
	if errMkdir := os.Mkdir(codexDir, 0o755); errMkdir != nil {
		t.Fatalf("failed to create codex dir: %v", errMkdir)
	}
	if errWrite := os.WriteFile(filepath.Join(authDir, "root.json"), []byte(`{"type":"claude"}`), 0o644); errWrite != nil {
		t.Fatalf("failed to write root auth: %v", errWrite)
	}
	if errWrite := os.WriteFile(filepath.Join(codexDir, "access-only.json"), []byte(`{"content":{"type":"oauth","provider":"openai-codex","access":"token"}}`), 0o644); errWrite != nil {
		t.Fatalf("failed to write nested auth: %v", errWrite)
	}

	w := &Watcher{}
	if got := w.loadFileClients(&config.Config{AuthDir: authDir}); got != 2 {
		t.Fatalf("expected 2 auth files, got %d", got)
	}
}
