...
func (w *Watcher) reloadClients(rescanAuth bool, affectedOAuthProviders []string, forceAuthRefresh bool) {
	...
	if rescanAuth {
		...
		if resolvedAuthDir, errResolveAuthDir := util.ResolveAuthDir(cfg.AuthDir); errResolveAuthDir != nil {
			...
			errWalk := filepath.WalkDir(resolvedAuthDir, func(fullPath string, entry os.DirEntry, errWalk error) error {
			...
			if errWalk != nil {
				log.Errorf("failed to read auth directory for hash cache: %v", errWalk)
			}
		}
		w.clientsMutex.Unlock()
	}
...
func (w *Watcher) loadFileClients(cfg *config.Config) int {
	...
	errWalk := filepath.WalkDir(authDir, func(fullPath string, entry os.DirEntry, errWalk error) error {
		if errWalk != nil || entry == nil || entry.IsDir() {
			return nil
		}
		name := entry.Name()
		...
		authFileCount++
		log.Debugf("processing auth file %d: %s", authFileCount, fullPath)
		if data, errReadFile := os.ReadFile(fullPath); errReadFile == nil && len(data) > 0 {
			successfulAuthCount++
		}
		return nil
	...
	if errWalk != nil {
		log.Errorf("error reading auth directory: %v", errWalk)
		return 0
	}
