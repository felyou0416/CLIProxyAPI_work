...
func TestApplyCodexHeadersUsesConfigUserAgentForOAuth(t *testing.T) {
	...
	if got := req.Header.Get("OpenAI-Beta"); got != "responses=experimental" {
		t.Fatalf("OpenAI-Beta = %s, want responses=experimental", got)
	}
...
func TestApplyCodexHeadersSetsOAuthCodexCLIHeaders(t *testing.T) {
	...
	if got := req.Header.Get("Authorization"); got != "Bearer access-token" {
		t.Fatalf("Authorization = %s, want Bearer access-token", got)
	}
	if got := req.Header.Get("Accept"); got != "text/event-stream" {
		t.Fatalf("Accept = %s, want text/event-stream", got)
	}
	if got := req.Header.Get("OpenAI-Beta"); got != "responses=experimental" {
		t.Fatalf("OpenAI-Beta = %s, want responses=experimental", got)
	}
	...
	if got := req.Header.Get("Chatgpt-Account-Id"); got != "acct-chatgpt" {
		t.Fatalf("Chatgpt-Account-Id = %s, want acct-chatgpt", got)
	}
}

func TestCodexExecutorPrepareRequestSetsOAuthCodexCLIHeaders(t *testing.T) {
	req, err := http.NewRequest(http.MethodPost, "https://chatgpt.com/backend-api/codex/responses", nil)
	if err != nil {
		t.Fatalf("NewRequest() error = %v", err)
	}
...
func TestApplyCodexHeadersDoesNotOverrideAPIKeyOriginator(t *testing.T) {
	req, err := http.NewRequest(http.MethodPost, "https://example.com/responses", nil)
	if err != nil {
		t.Fatalf("NewRequest() error = %v", err)
	}
	...
	if got := req.Header.Get("Originator"); got != "Client App" {
		t.Fatalf("Originator = %s, want Client App", got)
	}
	if got := req.Header.Get("User-Agent"); got != "client-ua" {
		t.Fatalf("User-Agent = %s, want client-ua", got)
	}
}
