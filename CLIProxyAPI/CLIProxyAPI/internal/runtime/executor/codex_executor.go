...
func (e *CodexExecutor) PrepareRequest(req *http.Request, auth *cliproxyauth.Auth) error {
	...
	if token = strings.TrimSpace(token); token != "" {
		...
		if !codexAuthUsesAPIKey(auth) {
			applyCodexOAuthHTTPHeaders(req, auth, e.cfg)
		}
	}
...
func applyCodexHeaders(r *http.Request, auth *cliproxyauth.Auth, token string, stream bool, cfg *config.Config) {
	...
	if codexAuthUsesAPIKey(auth) {
		...
		applyCodexOAuthHTTPHeaders(r, auth, cfg)
	}
	if stream {
...
func applyCodexOAuthHTTPHeaders(r *http.Request, auth *cliproxyauth.Auth, cfg *config.Config) {
	if r == nil {
		return
	}
	var ginHeaders http.Header
	...
	if auth != nil && auth.Metadata != nil {
		...
		} else if accountID, ok := auth.Metadata["chatgpt_account_id"].(string); ok {
			if trimmed := strings.TrimSpace(accountID); trimmed != "" {
				r.Header.Set("Chatgpt-Account-Id", trimmed)
			}
		}
