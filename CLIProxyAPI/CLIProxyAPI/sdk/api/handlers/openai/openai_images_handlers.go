...
func openAICompatImagesUpstreamModel(model string) string {
...
func (h *OpenAIAPIHandler) ImagesGenerations(c *gin.Context) {
	...
	if isOpenAICompatImagesModel(imageModel) {
		compatReq := buildOpenAICompatImagesJSONRequest(rawJSON, openAICompatImagesUpstreamModel(imageModel), stream)
...
func (h *OpenAIAPIHandler) imagesEditsFromMultipart(c *gin.Context) {
	...
	if isOpenAICompatImagesModel(imageModel) {
		compatReq, contentType, errBuild := buildOpenAICompatImagesMultipartRequest(form, openAICompatImagesUpstreamModel(imageModel), stream)
		if errBuild != nil {
			c.JSON(http.StatusBadRequest, handlers.ErrorResponse{
				Error: handlers.ErrorDetail{
					Message: fmt.Sprintf("Invalid request: %v", errBuild),
