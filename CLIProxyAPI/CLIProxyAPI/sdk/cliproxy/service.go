...
func (s *Service) registerModelsForAuth(a *coreauth.Auth) {
	...
	default:
		...
		if s.cfg != nil {
			...
			if isCompatAuth {
				// No matching provider found in configuration; try loading models directly from the credential JSON.
...
func buildOpenAICompatibilityConfigModels(compat *config.OpenAICompatibility) []*ModelInfo {
	...
	for i := range compat.Models {
		...
		if model.Image || strings.EqualFold(modelName, "gpt-image-2") {
			modelType = registry.OpenAIImageModelType
		}
		thinking := model.Thinking
		if thinking == nil && modelType != registry.OpenAIImageModelType {
